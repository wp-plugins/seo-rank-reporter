...
function kw_seo_rank_menu(){
	...
	add_submenu_page('seo-rank-reporter', 'Settings', 'Settings', 'administrator', 'seo-rank-settings', 'kw_seo_settings_menu');
...
function kw_rank_checker($target_key,$entered_url,$first_time) {
	...
	foreach($google_url_array as $value) {
		...
		if($first_time) {
			$results = array(htmlspecialchars(urldecode($target_keyword), ENT_QUOTES), htmlspecialchars($entered_url, ENT_QUOTES), 'Google', date('Y-m-d'), '-1', '-1');
		} else {
